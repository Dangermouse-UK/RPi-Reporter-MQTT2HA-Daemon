...
rpi_last_install_date = datetime.min
rpi_updates_available = False
...
def getLastUpdateDate():
    ...
    # Hold local variable of update timestamp
    latest_update_date_check = fileModDate.replace(tzinfo=local_tz)
    print_line('Latest update check=[{}]'.format(latest_update_date_check), debug=True)
    # If it has changed from last time we checked then an apt-update has run so
    # updates may potentially be available for which we need to check
    if latest_update_date_check != rpi_last_update_date:
        print_line('Updates vailable',debug=True)
        rpi_updates_available=True
    else:
        print_line('No updates available',debug=True)
        rpi_updates_available=False
...
def getLastInstallDate():
    ...
    if len(line_parts) > 1:
        ...
        # Store the date of the last time we ran apt update
        rpi_last_install_date = pkg_install_date

    print_line('rpi_last_install_date=[{}]'.format(
        rpi_last_install_date), debug=True)
...
def send_status(timestamp, nothing):
    ...
    if rpi_last_install_date != datetime.min:
        rpiData[K_RPI_DATE_LAST_UPDATE] = rpi_last_install_date.astimezone().replace(
            microsecond=0).isoformat()
    else:
        rpiData[K_RPI_DATE_LAST_UPDATE] = ''
...
def update_values():
    ...
    getLastInstallDate()
    getLastUpdateDate()
    if rpi_updates_available == True:
        # Updates may be available as apt update has been run since last time so lets go check
        getNumberOfAvailableUpdates() # and count them!
        print_line('Getting available updates', debug=True)
    getDeviceMemory()
    getNetworkIFs()
    ...
    while True:
        ...
        if apt_available:
            if timeNow > update_last_fetch_time + kUpdateCheckIntervalInSeconds:
                # Lets just go check anyway periodically in case apt update is not being run in the background
                # although this only does an apt upgrade so is not really accurate
                getNumberOfAvailableUpdates() # and count them!
                print_line('Periodic update check',debug=True)
